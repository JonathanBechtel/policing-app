...
def generate_shap_chart_data(sample, pipeline, explainer, search_val=False, search_outcome=False):
    ...
    if search_val:
        search_cols = [col for col in new_sample.columns if 'Search' in col]
        if not search_outcome:
            new_sample.columns = ['City', 'Age', 'Race', 'Sex', 'Reason For Stop'] + search_cols + ['Hour', 'Day', 'Quarter']
        else:
            new_sample.columns = ['City', 'Age', 'Race', 'Sex', 'Contraband Found', 'Reason For Stop'] + search_cols + ['Hour', 'Day', 'Quarter']
            
    col_vals = new_sample.values[0].tolist()
    col_vals = [val.title() if type(val) == str else int(val) for val in col_vals]
    new_sample.columns = [f"{col}: {value}" if  col not in search_cols else col for col, value in zip(new_sample.columns, col_vals)]
    if search_outcome:
